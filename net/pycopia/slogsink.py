...
# 
# $Id: slogsink.py 127 2009-02-04 13:03:43Z keith.dart $
#
...
class SlogDispatcher(socket.AsyncSocket):
    ...
    def error_handler(self, ex, val, tb):
        print("*** Dispatcher:", ex, val, file=sys.stderr)
...
class UserSlogDispatcher(socket.AsyncSocket):
    ...
    def read(self):
        ...
        except socket.SocketError as err:
            if err[0] == EAGAIN:
                return
            else:
                raise
...
class Syslog(object):
    ...
    def openlog(self, fname):
        ...
        try:
            ...
            print("Warning: could not open %r for writing: %s (%s)." % (fname, ex, val), file=sys.stderr)
        else:
            self._FLIST.append(fp)
...
port to listen to.
    ...
    sl.message("STARTING", ", ".join([o.name for o in sl.logfiles]))
