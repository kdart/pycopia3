...
from pycopia.http import useragents
...
def get_page(url, data=None, logfile=None, cookiefile=None, retries=1, filename=None, **kwargs):
    ...
    Args::
        ...
        filename    : Name of file to write target object directly to.
                      The body attribute will not be available in the
...
class HTTPResponse(object):
    ...
    def __unicode__(self):
        try:
            return str(self._make_body(), self.requested_encoding, "strict")
    ...
    def _make_body(self):
        if self._body:
            ...
            self._body = b"".join(self._contents)
    ...
    def _make_status(self):
        ...
        try:
            ...
            raise BadStatusLineError(line)
        try:
            version = float(version.split(b"/")[1])
...
class HTTPRequest(Request):
    ...
    def __str__(self):
        ...
        s.extend(list(map(str, self._headers)))
        return "\n".join(s)
    ...
    def get_getter(self):
        ...
        c.setopt(c.HTTPHEADER, list(map(str, self._headers)))
        self._set_common(c)
        return c, resp
    ...
    def get_poster(self):
        ...
        elif hasattr(self._data, "to_json"):
            return self.get_json_poster()
        else:
    ...
    def set_proxy(self, value):
        ...
        if urls.URI_RE_STRICT.search(value) is None:
            self._proxy = None
...
class HTTPConnectionManager(ConnectionManager):
    ...
    def get_URL(self, obj, logfile=None, cookiefile=None, **kwargs):
        ...
        """
        obj_t = type(obj)
        if issubclass(obj_t, (str, urls.UniversalResourceLocator)):
    ...
    def post_URL(self, obj, data, logfile=None, **kwargs):
        ...
        if issubclass(obj_t,  (str, urls.UniversalResourceLocator)):
            ...
            for url, rd in zip(iter(obj), iter(data)):
...
if __name__ == "__main__":
    ...
    p = get_page("http://www.google.com/")
    print(p.status)
    print(p.body)
