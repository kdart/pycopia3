...
class ImpairmentsConfigurer:
    ...
    def clear(self, interface):
        ...
        try:
            ...
            if rerr.args[0] == 2:  # OK, Indicates it was already clear.
                return
            else:
                raise
    ...
    def delay(self, interface, milliseconds, jitter=None, correlation=None,
              distribution=None):
        ...
        correlation : float (percent), optional
            This causes the next random delay to be this percentage near the
            last one.
        ...
        Notes
        -----
        The correlation and distribution parameters are mutually exclusive.
        Supply only one.
    ...
    def reorder(self, interface, percent, delay=10, correlation=0):
        self.clear(interface)
        cmd = "tc qdisc add dev {0} root netem delay {1:3.0d}ms reorder {2} {3}".format(  # noqa
            interface,
            delay,
    ...
    def corrupt(self, interface, percent):
        self.clear(interface)
        cmd = "tc qdisc add dev {0} root netem corrupt {1:2.1f}%".format(
            interface, percent)
        return self._perform(cmd)

    def duplication(self, interface, percent):
        self.clear(interface)
        cmd = "tc qdisc add dev {0} root netem duplicate {1:2.1f}%".format(
            interface, percent)
        return self._perform(cmd)
    ...
    def add_corrupt(self, interface, percent):
        cmd = "tc qdisc change dev {0} root netem corrupt {1:2.1f}%".format(
            interface, percent)
        return self._perform(cmd)

    def add_duplication(self, interface, percent):
        cmd = "tc qdisc change dev {0} root netem duplicate {1:2.1f}%".format(
            interface, percent)
        return self._perform(cmd)

    def add_reorder(self, interface, delay=10, percent=10, correlation=None):
        cmd = "tc qdisc change dev {0} root netem delay {1:3.0d}ms reorder {2} {3}".format(  # noqa
            interface,
            delay,
            "{0:2.1f}%".format(float(percent)),
            "{0:2.1f}%".format(correlation) if correlation else "",
    ...
    def current(self):
        impairments = list(map(self.show, self.interfaces))
        if impairments:
    ...
    def set_impairment(self, latency=None, delay=None, jitter=None,
        ...
        imp = self.get_impairment(latency, delay, jitter,
                                  loss, drop, corrupt, reorder, duplicate,
                                  correlation)
        imp.applyto(self)
        return imp
...
class Option:
    ...
    def __init__(self, value, **options):
        if isinstance(value, self.__class__):
            ...
            if self._possible_values and v not in self._possible_values:
                raise ValueError("{!r} is not one of {!r}".format(
                    v, self._possible_values))
            self._value = v
            ...
            self.setup(options)
            if options:
                raise ValueError("Option got extra options: {!r}".format(
                    list(options.keys())))
    ...
    def __repr__(self):
        ...
        return "{0}({1!r}{2}{3})".format(
            ...
            ", " if attrs else "", ", ".join("{}={!r}".format(
                name, getattr(
                    self, name, default)) for (name, bt, default) in attrs))

    def __str__(self):
        return "{} {}".format(
            self.__class__.__name__.lower(), self._get_option_string())
...
class Distribution(Option):
    _basetype = str
    _possible_values = (DIST_UNIFORM, DIST_NORMAL, DIST_PARETO,
                        DIST_PARETONORMAL)
