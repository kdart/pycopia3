...
class HTTPResponse(object):
    ...
    def _make_status(self):
        ...
        try:
            ...
            version = 0.9
        reason = reason.strip()
        reason = reason or httputils.STATUSCODES.get(status, b"")
        self._responseline = None
...
class HTTPRequest(Request):
    ...
    def get_URLencoded_poster(self):
        ...
        Returns a tuple of initialized Curl and HTTPResponse objects.
        """
        data = urls.urlencode(self._data, True)
        c = pycurl.Curl()
        resp = HTTPResponse(self._encoding)
        c.setopt(c.URL, str(self._url))
    ...
    def reset(self, url, data=None, query=None, method="GET",
        ...
        if cookiejar:
            ...
            if c:
                h.append(c)
        h.append(httputils.UserAgent(useragents.get_useragent(useragent, useragent)))
