...
from pycopia.QA.db import types
...
class CidrInput(BaseInput):
    ...
    def validate(self):
        try:
            ipaddress.IPv4Network(self.wid.edit_text)
        except ValueError:
            return False
        return True
...
class InetInput(BaseInput):
    ...
    def validate(self):
        try:
            ipaddress.IPv4Interface(self.wid.edit_text)
        except ValueError:
            return False
        return True
...
class RelationshipInput(BaseInput):
    ...
    def value(self):
        if self.metadata.uselist:
            return list(self.dbvalue.values()) # list of foriegn key ids
        else:
            return self.dbvalue
    ...
    def _get_top_label(self):
        if self.metadata.uselist:
            label = "{{{}}} (Edit)".format(", ".join(str(o) for o in list(self.dbvalue.values())))
        else:
    ...
    def _create_relation_input(self):
        ...
        if self.metadata.nullable:
            ...
            wlist.append(entry)
        for pk, cname in list(choices.items()):
...
class AttributeInput(BaseInput):
        ...
        for attrib in getattr(self.row, self.metadata.colname): # list-like attribute
            entry = ListEntry(urwid.Columns(
                    [(30, urwid.Text(str(attrib.type))),
                     urwid.Text(str(attrib.value).encode("utf-8"))]))
    ...
    def _edit_attribute(self, entry):
        ...
        frm = AttributeEditForm(entry.attrname, str(value).encode("utf-8"))
    ...
    def _edit_ok(self, form, newtext, entry):
        ...
        saveentry.base_widget[1].base_widget.set_text(str(newval).encode("utf-8"))
        entry._w = saveentry
...
class TestEquipmentInput(BaseInput):
    ...
    def _stringify_te(self, te):
        if te.roles:
            ts = "{}  roles: {}".format(te, ", ".join(str(role) for role in te.roles))
        else:
            ts = str(te)
...
class EquipmentInterfaceInput(BaseInput):
    ...
    def build(self):
        ...
        for intf in list(self.equipmentrow.interfaces.values()):
            wlist.append(self._get_intf_entry(intf))
        listbox = urwid.ListBox(urwid.SimpleFocusListWalker(wlist))
        return urwid.BoxAdapter(urwid.LineBox(listbox), max(7, len(wlist)+2))

    def _get_intf_entry(self, intf):
        entry = ListEntry(urwid.Text(str(intf).encode("utf-8")))
    ...
    def _edit_complete(self, form, pkval, listentry):
        ...
        listentry._w.base_widget.set_text(str(listentry.interface).encode("utf-8"))
        #self._invalidate()
        form._emit("popform")
...
class DeleteDialog(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
    signals = ["ok", "cancel"]
...
class AttributeEditForm(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
    signals = ["ok", "cancel"]
...
class AttributeAddForm(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
    signals = ["ok", "cancel"]
...
class TestEquipmentAddForm(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
...
class TestEquipmentEditForm(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
...
class InterfaceAttachForm(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
...
class ListScrollSelector(urwid.Widget):
    ...
    def __init__(self, choicelist):
        ...
        maxwidth = reduce(lambda c,m: max(c,m), [len(str(o)) for o in choicelist], 0)
...
class MultiselectListForm(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
    signals = ["popform", "message"]
    ...
    def _build_list(self):
        ...
        for pk, cobj in list(choices.items()):
            if pk in current:
...
class MultiselectForm(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
    signals = ["popform", "message"]
...
#### top level forms
class Form(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
...
class GenericListForm(Form):
    ...
    def get_items(self):
        ...
        if self._filter:
            filt = []
            for name, value in list(self._filter.items()):
    ...
    def _create_cb(self, b):
        ...
        urwid.connect_signal(cform, 'pushform', lambda oform, newform:
                self._emit("pushform", newform))
...
class YesNoDialog(urwid.WidgetWrap, metaclass=urwid.MetaSignals):
    signals = ["ok", "cancel", "popform", "message"]
...
#        frm = urwid.Frame(listbox, header=header)
        ...
        leftlist = []
        rightlist = []
        for pk, row in list(choices.items()):
            entry = ListEntry(urwid.Text(str(row).encode("utf-8")))
            #urwid.connect_signal(entry, 'activate', self._single_select, pk)
            leftlist.append(entry)

        for pk, row in list(currentvalues.items()):
            entry = ListEntry(urwid.Text(str(row).encode("utf-8")))
            #urwid.connect_signal(entry, 'activate', self._single_select, pk)
