...
"""
Simple CLI interface to QA database.
    ...
    """
        ...
        if cmd.startswith("add"):
            ...
            try:
                ...
                for intf in list(self._obj.interfaces.values()):
                    self._print(intf)
            else:
...
def new_relation_input(ui, modelclass, metadata):
    ...
    if not choices:
        ...
        if metadata.uselist:
            ...
            return None
    if metadata.uselist:
        return list(ui.choose_multiple_from_map(dict(choices), None, "%%I%s%%N" % metadata.colname).keys())
    else:
        if metadata.nullable:
...
def edit_relation_input(ui, modelclass, metadata, dbrow):
    ...
    if metadata.uselist:
        if metadata.collection == "MappedCollection":
            chosen = dict((crow.id, str(crow)) for crow in list(current.values()))
        else:
        ...
        if chosen:
            t = _session.query(relmodel).filter( relmodel.id.in_(list(chosen.keys()))).all()
        else:
            t = []
