...
class WindowsRemoteCLI(RemoteCLI):
    ...
    Get or show the files attributes. The possible attribute names may also be listed."""
        ...
        if cmd == "list":
            ...
            for attrname in list(attrmap.keys()):
        ...
        if cmd == "get":
            ...
            for name, flag in list(attrmap.items()):
                if flag & flags:
                    self._print(name)
        elif cmd == "set":
            newflags = flags
            toset = list(map(str.upper, argv[3:])) # this mess allows for abbreviated flag names
            for attrname, attrvalue in list(attrmap.items()):
            ...
            newflags = flags
            toset = list(map(str.upper, argv[3:]))
            for attrname, attrvalue in list(attrmap.items()):
                for setflag in toset:
                    if attrname.startswith(setflag):
                        newflags &= ~attrvalue
...
class TopLevelCLI(CLI.BaseCommands):
    ...
    Print available clients."""
        ...
        for name, uri in list(self._objs.items()):
