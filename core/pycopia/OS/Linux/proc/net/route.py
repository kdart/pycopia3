...
class RouteEntry(object):
    ...
    def __str__(self):
        return "\t".join([self.Iface]+list(map(_hex, [self.Destination, self.Gateway]))+list(map(str, [
                self.Flags, self.RefCnt, self.Use, self.Metric])) + [_hex(self.Mask)] +
                list(map(str, [self.MTU, self.Window, self.IRTT])))
...
class RouteTable(object):
    ...
    def __str__(self):
        return "\n".join([HEADER] + list(map(str, self._entries)))
