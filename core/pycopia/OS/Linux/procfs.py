...
class ProcStat(object):
    ...
    def __str__(self):
        if self.stats is None:
            ...
            names = list(self._STATINDEX.keys())
            names.sort()
            for name in names:
    ...
    def __iter__(self):
        ...
        for name in list(self._STATINDEX.keys()):
            i = self._STATINDEX[name]
            yield (name, self.stats[i])

    def get_attributes(self):
        return list(self._STATINDEX.keys())
...
A collection of all processes running, like the standard 'ps' command. """
    ...
    def __iter__(self):
        return iter(list(self._ptable.values()))
    ...
    def tree(self):
        class _pholder:
            pass
        self.read()
        if 0 not in self._ptable:
            p0 = self._ptable[0] = _pholder()
            p0.pid = p0.ppid = 0
            p0.cmdline = "<kernel>"
        for p in list(self._ptable.values()):
            try:
