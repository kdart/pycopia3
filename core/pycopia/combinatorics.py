...
def factorial(x):
    ...
    return x!
    """
    return x<=0 or reduce(lambda a,b: a*b, list(range(1,x+1)) )
fact = factorial
...
class PermuterIter(object):
    def __init__(self, seq):
        ...
        self.i = 0
        self.is_string = isinstance(seq, str)
...
class KeywordCounter(object):
    ...
    def __init__(self, **kwargs):
        ...
        self._counter = ListCounter(list(kwargs.values())) # values should be sequences
    ...
    def __next__(self):
        values = next(self._counter) # the list counter will raise StopIteration
    ...
    def fetch(self, values):
        return dict(list(zip(self._names, values)))
