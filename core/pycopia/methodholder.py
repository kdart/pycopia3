...
formal parameters for user interfaces.
    ...
    def __init__(self, method=None, **kwargs):
        ...
        if method is None:
            ...
            self.__dict__["_methodname"] = method.__func__.__name__
            # formal names
            varnames = list(method.__func__.__code__.co_varnames)[:method.__func__.__code__.co_argcount]
            varnames.reverse()
            df = method.__func__.__defaults__ or []
            defaults = list(df) ; defaults.reverse()
            for name, deflt in zip(varnames, defaults):
                self[name] = deflt
    ...
    def __str__(self):
        ...
        params = ", ".join(["%s=%r" % (n, v) for n, v in list(self.items())])
        return "%s: %s" % (m, params)

    def __repr__(self):
        return "%s(%r, %s)" % (self.__class__.__name__, self.__dict__["_methodname"],
        ", ".join(["%s=%r" % (n, v) for n, v in list(self.items())]) )
