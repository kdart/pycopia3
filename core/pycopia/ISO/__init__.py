...
def build_iso639a(filename):
    ...
    ccf = urllib.request.urlopen(req)

    outf = open(filename, "w")
    outf.write("""#!/usr/bin/python3.4
...
""")
    ...
    for line in ccf.readlines():
        ...
        if len(parts) != 5:
            continue
        [alpha3_bib, alpha3_term, alpha2, name_en, name_fr] = list(map(str.strip, parts))
        if alpha2:
            name_en = str(name_en, 'utf-8')
...
""")
    ...
    try:
        ccf = urllib.request.urlopen(URLS["countrycodes"])
    except:
